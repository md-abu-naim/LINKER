...
const NavLinks = () => {
    ...
    return (
        <ul className="hidden md:flex just items-center font-medium gap-6 lg:mr-24">
            {
                navLinks?.map((link) => (
                    <li key={link.href} title={link.title} className={`hover:text-cyan-400 ${pathname === link.href && 'text-cyan-500 border-cyan-600 border-b-2'} py-2 px-6 transition-colors`}>
                        <Link href={link.href} >
                            {link.icon}
                        </Link>
                    </li>
